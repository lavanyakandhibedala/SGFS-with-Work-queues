...
static int create_trashbin(void)
{
	...
	{
		printk("trashbin created successfully sys_xclone");
...
int processunlink(char *fname) /* only CLONE_PROT_MV */
{
...
int processencryption(char* fname, char* key){
	...
	if(!f_write || IS_ERR(f_write)){
        ...
        if (!key)
        {
                printk("\nERROR : Key for encryption is not set \n");
                return -EINVAL;
        }
	key_store = kmalloc(sizeof(char*)*strlen(key),GFP_KERNEL);
...
int compress_file(struct file * f_read, struct file * f_write)
{
	...
	mm_segment_t oldfs1;
	mm_segment_t oldfs2;
	int out_len = (int) kmalloc(sizeof(size_t),GFP_KERNEL);
    ...
    fsize = i_size_read(Inode);
 	while(fsize > 0){
                ...
                bytes2 = vfs_write(f_write, buff4, err, &f_write->f_pos);
...
long xclone2(void *arg) {
	...
	else if(tsk->xclone_flag == 0x000000fff){
		printk("We are purging all files\n");
		purge_err = deletefiles(param->fname);
		printk("Purge error is %d\n",purge_err);
		goto out;
	}

	printk("Param->b1 is %d\n",param->b1);
